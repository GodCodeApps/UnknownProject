package com.lib_common.widget.ninegrid;
...
public class NineGridView extends ViewGroup {
    ...
    private float singleImageRatio = 3f;            // 单张图片的宽高比(宽/高)
...
//                }
                gridWidth = QMUIDisplayHelper.getScreenWidth(getContext()) - getPaddingLeft() - getPaddingRight() - getResources().getDimensionPixelSize(R.dimen.sm_px_30);
                gridHeight = getResources().getDimensionPixelSize(R.dimen.sm_px_195);
            } else {
//                gridWidth = gridHeight = (totalWidth - gridSpacing * (columnCount - 1)) / columnCount;
    ...
    private ImageView getImageView(final int position) {
        ...
        if (position < imageViews.size()) {
            ...
            imageView.setOnClickListener(v -> mAdapter.onImageItemClick(getContext(), NineGridView.this, position, mAdapter.getImageInfo()));
    ...
    public ImageView getItemView(int position) {
        if (imageViews != null && imageViews.size() > position)
