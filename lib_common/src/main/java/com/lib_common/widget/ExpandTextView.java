...
public class ExpandTextView extends AppCompatTextView implements View.OnClickListener {
    ...
    private static final String FOLD_TIP_TEXT = "全部";
    ...
    public ExpandTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            TypedArray arr = context.obtainStyledAttributes(attrs, R.styleable.ExpandTextView);
            mShowMaxLine = arr.getInt(R.styleable.ExpandTextView_showMaxLine, MAX_LINE);
            mTipGravity = arr.getInt(R.styleable.ExpandTextView_tipGravity, END);
            mTipColor = arr.getColor(R.styleable.ExpandTextView_tipColor, TIP_COLOR);
            mTipClickable = arr.getBoolean(R.styleable.ExpandTextView_tipClickable, false);
            mFoldText = arr.getString(R.styleable.ExpandTextView_foldText);
            mExpandText = arr.getString(R.styleable.ExpandTextView_expandText);
            isShowTipAfterExpand = arr.getBoolean(R.styleable.ExpandTextView_showTipAfterExpand, false);
            isParentClick = arr.getBoolean(R.styleable.ExpandTextView_isSetParentClick, false);
            arr.recycle();
        }
    ...
    private void formatText(CharSequence text, final BufferType type) {
        mOriginalText = StringUtils.replaceBlank(text.toString()).replace(" ", "");
    ...
    private void translateText(Layout layout, BufferType type) {
        if (layout != null && layout.getLineCount() > mShowMaxLine) {
            ...
            if (mTipGravity == END) {
                ...
                while (layout.getPrimaryHorizontal(end - 1) + getTextWidth(mOriginalText.subSequence(end - 1, end).toString()) < x && end >= length) {
                    end++;
                }
                end -= 2;
            } else {
